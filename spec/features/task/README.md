...
> [SpecScore.**Studio**](https://specscore.studio): | [Explore](https://specscore.studio/app/github.com/specscore/specscore/spec/features/task?op=explore) | [Edit](https://specscore.studio/app/github.com/specscore/specscore/spec/features/task?op=edit) | [Ask question](https://specscore.studio/app/github.com/specscore/specscore/spec/features/task?op=ask) | [Request change](https://specscore.studio/app/github.com/specscore/specscore/spec/features/task?op=request-change) |
...
- **`complete` conflates merged, deployed, and verified — deferred, ruled 2026-08-27.** A task-status lane proposed deriving `complete` deterministically from evidence (PR merged / tag exists / worktree cleaned) instead of an agent asserting it. The founder declined: *"let's leave conflicts resolution to agents for now. Task can be merged but not deployed or not verified. Depends on definition of done."* So **evidence-based status derivation is not being built**; status-conflict resolution stays with agents for now. The substantive gap behind the proposal remains real and unresolved: the single terminal `complete` value cannot carry the distinction between a task's code being merged, its being deployed, and its being verified — and which of those counts as "done" depends on the definition of done in force for the effort. This is adjacent to but distinct from [unify-task-status-vocabulary](../unify-task-status-vocabulary/README.md) (Implemented), which unified the vocabulary's *tokens* but did not address sub-dividing `complete` by evidence stage. Any future proposal to re-derive `complete` from merge/deploy/verify evidence should be checked against this ruling first.
